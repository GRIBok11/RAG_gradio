...
def create_new_chat(username: str, history: List[Tuple[str, str]]) -> None:
    ...
    if user:
        chat_history = ChatHistory(id_user=user.id, chat_content=json.dumps(history))
...
def save_chat_history(username: str, history) -> None:
    ...
    if user:
        ...
        chat_history = session.query(ChatHistory).filter(ChatHistory.user_id == user.id).first()
...
def save_chat_history(username: str, history: List[Tuple[str, str]]) -> None:
    ...
    if user:
        # Ищем запись чата по chat_id и проверяем, что этот чат принадлежит пользователю
        chat_history = session.query(ChatHistory).filter(ChatHistory.id_user == user.id).first()
...
def load_last_chat(username: str) -> Tuple[List[Tuple[str, str]], int]:
    ...
    if user:
        ...
        last_chat = session.query(ChatHistory).filter(ChatHistory.id_user == user.id).order_by(ChatHistory.created_at.desc()).first()
        if last_chat:
